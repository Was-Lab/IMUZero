...
def save_checkpoint(epoch, model, optimizer, filename='data/wurenji/pth/pic_abs/checkpoint_18npy_one.pth'):
    state = {
        'epoch': epoch,
...
if os.path.exists('data/wurenji/pth/pic_abs/checkpoint_18npy_one.pth'):
    checkpoint = torch.load('data/wurenji/pth/pic_abs/checkpoint_18npy_one.pth')
...
if flag == "train":
    for epoch in range(start_epoch, num_epochs):
        ...
        if (epoch + 1) % 5 == 0:
            ...
            plt.savefig(f'data/wurenji/pth/pic_abs/confusion_matrix_resnet_npy_{epoch}.png')  # 将图像保存为文件
            plt.show()

            # 保存模型权重-----------------------------------------------------------------------------------------------------
            if accuracy > best_accuracy:
                best_accuracy = accuracy
                torch.save(model.state_dict(), 'data/wurenji/pth/pic_abs/best_model.pth')
...
# /////////////////////////////////////////////////////////////////////////////////////////直接做测试集
    ...
    # 加载预训练的ResNet18模型，这里我们指定pretrained=False因为我们手动加载权重
    model = models.resnet18(pretrained=False)
    ...
    model.load_state_dict(torch.load('data/wurenji/pth/pic_abs/best_model.pth'))
