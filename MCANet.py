...
def conv1x1(in_planes, out_planes, stride=1):  # 用于改变特征图的通道数
...
class MACANet(nn.Module):
    def __init__(self, num_classes = 1000, pretrained = True, backbone = 'ResNet101', att_type=None):
        ...
        # print(num_classes)
        self.encoder = EncoderBlock(pretrained, backbone, att_type=att_type)  # 编码器
        # self.decoder = DecoderBlock(num_classes)  # 解码器


    def forward(self, sar_img, opt_img):
        opt_sar_low_high_features = self.encoder.forward(sar_img, opt_img)
        # classification = self.decoder(opt_sar_low_high_features)  # 分类结果
...
class EncoderBlock(nn.Module):
    def __init__(self, pretrained = True, backbone = 'ResNet101', num_classes=1000, att_type=None):
        ...
        if backbone == 'ResNet101':
            self.SAR_resnet = resnet101(pretrained, type='sar', num_classes=num_classes, att_type=att_type)  # 两个 ResNet101 模型分别处理 SAR 和光学图像
    ...
    def forward(self, sar_img, opt_img):
        ...
        # sar图
        sar_low_feat = sar_feats[1]  # ResNet
        sar_high_feat = sar_feats[4]
        sar_final_feat = self.conv1(sar_feats[4])
        # 光学图
        opt_low_feat = opt_feats[1]  # ResNet
        opt_high_feat = opt_feats[4]
        opt_final_feat = self.conv1(opt_feats[4])

        # 是通过 MCAM 模块融合 SAR 和光学图像的特征
        low_level_features = self.MCAM_low(sar_low_feat, opt_low_feat)  # MCAM
        high_level_features = self.MCAM_high(sar_high_feat, opt_high_feat)  # MCAM
